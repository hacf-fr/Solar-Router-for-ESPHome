...
- **Power Meters**: Measure grid energy exchange (Fronius, HA API, Proxy, etc.)
...
- `green_led_pin`, `yellow_led_pin` - LED GPIO pins
...
**Validation Tools:**
- Use `esphome check` for configuration validation
...
**GPIO Usage:**
- LED indicators: Typically GPIO18, GPIO19 (green, yellow)
- Triac gate: Various GPIOs (configurable)
- Zero-crossing: Typically GPIO23 (inverted)
- Temperature sensors: Various GPIOs
...
esphome validate path/to/config.yaml
...
2. Include `power_meter_common.yaml` as base
3. Implement `power_meter_source` script
...
2. Include `engine_common.yaml` as base
...
### Common Issues
- **YAML syntax errors**: Use `esphome validate` with full path
