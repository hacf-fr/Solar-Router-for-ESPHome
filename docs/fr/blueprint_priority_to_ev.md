...
surplus plutôt que le chauffe-eau piloté par le routeur solaire. Ce
blueprint arbitre entre les deux :

- Lorsqu'un surplus *suffisant* est disponible *assez longtemps*, il
  éteint le routeur solaire pour que le surplus soit relâché vers le
...
- Lorsque le VE est débranché, qu'un nuage coupe la production PV et
  ...
  rallume le routeur solaire pour que le chauffe-eau récupère ce qui
  reste.
...
- `grid_power` est signé : **positif à l'import**, **négatif à
  l'export**. C'est la convention par défaut du firmware du routeur solaire
  (`power_sign: "1"`).
